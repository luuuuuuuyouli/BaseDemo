...
-(void)setupUI
{
    ...
    self.dateView=[[UIView alloc] initWithFrame:CGRectMake(W(30), (SCREEN_H- H(550))/2, SCREEN_W-W(60), H(550))];
    ...
    self.chooseTimeLabel.text=@"请选出生日期";
    self.chooseTimeLabel.textColor= RGB16(0x333333);
    self.chooseTimeLabel.font=[UIFont systemFontOfSize:17];
    ...
    [self addLabelTextWithName:@[@"year",@"month",@"day"]];
    ...
    self.line3.backgroundColor = [UIColor clearColor];
    ...
    [self.okButton setTitle:@"Confirm" forState:UIControlStateNormal];
    //    [self.okButton setTintColor:RGBA(210, 55, 61, 1.0)];
    [self.okButton setTintColor:mainColor];
...
- (void)addLabelTextWithName:(NSArray *)nameArray{
    ...
    for (int i = 0; i < nameArray.count; i ++) {
        UILabel *label=[[UILabel alloc] initWithFrame:CGRectMake(W(50)+((self.width-W(150))/3*i), self.line1.bottom,(self.width-W(200))/3, H(60))];
...
{
    if (sender.tag == 101) {
        NSComparisonResult result = [[NSDate date] compare:_startDate];
        if (result != NSOrderedDescending) {
           // [MBProgressHUD show:@"设置时间不能大于当前时间"];
            showError(@"Less than today")
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
   // [self fadeOut];
    ...
    CALayer *layer=[self.dateView.layer hitTest:point];
    if (layer == nil) {
        [self fadeOut];
    }
}
